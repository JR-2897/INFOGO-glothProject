import os
from .models import Pathology, User, Treatment
...
#Doit renvoié la liste des traitements en fonction de l'id de la pathologie
def treatment(id):
    temp = Treatment.query.with_entities(Treatment.icd_10, Treatment.cis).get(id)
    choices = [(x[0], x[1]) for x in temp]
    return choices
